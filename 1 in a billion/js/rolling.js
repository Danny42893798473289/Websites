...
import { markEggDiscovered, markShinyDiscovered } from "./state.js";
...
export async function applyOfflineProgress({ loginBoot = false } = {}) {
  ...
  const offlineRps = getCurrentRPS();
  if (offlineRps <= 0) {
    if (runtime.el.offlineInfo) {
  ...
  try {
    await performRollBatched(rollsNow, false);
    if (rollsLater > 0) {
      runtime.pendingOfflineRolls += rollsLater;
    }
...
export function processPendingOfflineRolls() {
  if (!runtime.state || runtime.offlineProgressRunning || runtime.pendingOfflineRolls <= 0) {
    return false;
  }
  const chunk = Math.min(
    runtime.isMobile ? OFFLINE_PENDING_CHUNK_MOBILE : OFFLINE_PENDING_CHUNK_DES,
    runtime.pendingOfflineRolls
  );
...
export function maybeGetEgg(minRarityIndex = null) {
...
export function doLuckyRoll() {
  ...
  const now = Date.now();
  if (now < runtime.state.luckyRollAvailableAt) {
    const remaining = runtime.state.luckyRollAvailableAt - now;
    setFeed(`Lucky Roll cooling down: ${formatDuration(remaining)} remaining.`);
    return;
  }
  if (runtime.state.gems < LUCKY_ROLL_COST_GEMS) {
    setFeed(`Lucky Roll needs ${formatNumber(LUCKY_ROLL_COST_GEMS)} gems.`);
    return;
  }

  runtime.state.gems -= LUCKY_ROLL_COST_GEMS;
  runtime.state.luckyRollAvailableAt = now + LUCKY_ROLL_COOLDOWN_MS;
  ...
  runtime.state.totalRolls += 1;
  runtime.state.rollsSincePrestige += 1;
  if (runtime.el.lastRoll) runtime.el.lastRoll.textContent = String(die);

  const luckyBoost = getSetBonusValue("lucky") + getGlobalEventBonus("lucky");
  const bonusTierSteps = Math.min(5, Math.floor(luckyBoost * 10));
  const rareIndex = Math.max(0, getRarityIndex("Rare") + bonusTierSteps);
  const forcedEgg = maybeGetEgg(rareIndex);
  if (forcedEgg) {
    ...
    if (runtime.el.lastEgg) {
      runtime.el.lastEgg.innerHTML =
        ...
        `${forcedEgg.shiny ? "Shiny " : ""}${escapeHtml(forcedEgg.name)} (Lucky Roll, 1 in ${formatNumber(forcedEgg.oneIn)})`;
    }
    setFeed(`Lucky Roll found ${forcedEgg.shiny ? "Shiny " : ""}${forcedEgg.name} [${forcedEgg.rarity}] and earned ${formatNumber(coinGain)} coins!`);
...
export function getCoinMultiplier() {
  ...
  const mult = coinMultBonus * gemCoinBonus * prestigeBonus * setCoin * companionCoin * eventCoin * ascCoin;
