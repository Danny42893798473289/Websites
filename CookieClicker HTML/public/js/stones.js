...
function isStonesUnlocked(){
  if(typeof isLimboNoForge === 'function' && isLimboNoForge()) return false;
...
const STONE_REMOVE_COST = 1000000;

function removeBuildingStone(key){
  if(!isStonesUnlocked()) return;
  if(cookies < STONE_REMOVE_COST){ alert(`Removing stones costs ${STONE_REMOVE_COST.toLocaleString()} cookies.`); return; }
  cookies -= STONE_REMOVE_COST;
  ensureStoneData();
  const oldTier = Number(appliedBuildingStones[key]) || 0;
  if(oldTier <= 0) return;
