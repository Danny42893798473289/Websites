...
const MASTERY_NODES = [
  ...
  { id: 'income_root', path: 'fortune', row: 0, col: 1, cost: 15, requires: [], label: '2× income', desc: 'Double building cookie production.' },
  { id: 'income_4', path: 'fortune', row: 1, col: 1, cost: 30, requires: ['income_root'], label: '4× income', desc: 'Quadruple building production (replaces 2×).' },
  { id: 'income_10', path: 'fortune', row: 2, col: 1, cost: 60, requires: ['income_4'], label: '10× income', desc: '10× building production (replaces 4×).' },

  { id: 'pets_mult2', path: 'pets', row: 0, col: 1, cost: 15, requires: [], label: '2× pet power', desc: 'Equipped pets count double toward multiplier.' },
  { id: 'pets_mult4', path: 'pets', row: 1, col: 1, cost: 30, requires: ['pets_mult2'], label: '4× pet power', desc: '4× pet multiplier contribution (replaces 2×).' },
  { id: 'pets_mult10', path: 'pets', row: 2, col: 1, cost: 60, requires: ['pets_mult4'], label: '10× pet power', desc: '10× pet multiplier (replaces 4×).' },
  { id: 'pets_mult15', path: 'pets', row: 3, col: 1, cost: 100, requires: ['pets_mult10'], label: '15× pet power', desc: '15× pet multiplier (replaces 10×).' }
];
...
function getWhyEggCap(){
  if(typeof isLimboNoBonusCap === 'function' && isLimboNoBonusCap()) return 200;
  const lvl = Number(tornadoPrestige) || 0;
  if(lvl >= 3) return 500 + 20 * (Number(xp) || 0);
  return 200 + 10 * (Number(xp) || 0);
}
...
function hasMastery(id){
  if(typeof isLimboNoMastery === 'function' && isLimboNoMastery()) return false;
  return masteryUnlocked.includes(id);
}

function getBuildingMult(){
  if(hasMastery('income_10')) return 10;
  if(hasMastery('income_4')) return 4;
  if(hasMastery('income_root')) return 2;
  return 1;
}

function getPassiveMult(){ return 1; }

function getPetMasteryMult(){
  if(hasMastery('pets_mult15')) return 15;
  if(hasMastery('pets_mult10')) return 10;
  if(hasMastery('pets_mult4')) return 4;
  if(hasMastery('pets_mult2')) return 2;
  return 1;
}
...
function canUnlockMastery(id){
  ...
  if(!node || masteryUnlocked.includes(id)) return false;
