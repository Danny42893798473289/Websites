...
function formatNum(value){
  ...
  for(const unit of compact){
    if(abs >= unit.v){
      ...
      return `${scaled.toFixed(decimals).replace(/\.0+$/,"").replace(/(\.\d*[1-9])0+$/,"$1")}${unit.s}`;
    }
  }
...
function setSessionHeader(){
  ...
  if(userEl) userEl.textContent = currentUser ? `@${currentUser}` : "Guest";
}
...
function setSaveIndicator(state){
  ...
  el.dataset.state = state in map ? state : "idle";
...
setTextAnimated("cookies", formatNum(Math.floor(cookies)));
setTextAnimated("multi", getMulti().toFixed(2));
setTextAnimated("per", formatNum(typeof getPer40mPotential === 'function' ? getPer40mPotential() : Math.floor(getBase()*getMulti()*getPassiveMult())));

const setShopCount = (id, val) => { const el = document.getElementById(id); if(el) el.textContent = formatNum(val); };
setShopCount('g', grandmas);
setShopCount('f', farms);
setShopCount('fa', factories);
setShopCount('o', oils);
setShopCount('m', mines);
setShopCount('c', coops);
setShopCount('w', wormholes);
setShopCount('ga', galaxies);
setShopCount('e', empties);
...
if(seeds.length === 0){
  ...
  seeds.forEach((s,i)=>{
    ...
    const plantBtn = document.createElement('button'); plantBtn.className='primary'; plantBtn.textContent='Plant'; plantBtn.onclick = ()=>{ showMenu('garden'); showToast('Click an empty garden plot to plant the seed.'); };
...
if(produce.length === 0){
  ...
  produce.forEach((item,i)=>{
    ...
    const text = document.createElement('span'); text.textContent = `${emoji} ${item.type} x${count} (${formatNum(item.value)} each)`;
...
if(typeof updateIceAgeUI === 'function') updateIceAgeUI();
if(typeof updatePortalShopUI === 'function') updatePortalShopUI();
if(typeof updateRaritiesUI === 'function') updateRaritiesUI();
if(typeof updateTradeUI === 'function') updateTradeUI();
renderMastery();
renderExpansionTab();
...
const cb = document.getElementById('chokeberryCount');
if(cb) cb.textContent = String(chokeberries || 0);
const yp = document.getElementById('ypHud');
if(yp) yp.textContent = formatNum(yellowPrestige || 0);
const ss = document.getElementById('scoreSHud');
if(ss && typeof getScoreS === 'function') ss.textContent = getScoreS().toFixed(2);
try {
  if(typeof checkForcedIceAge === 'function') checkForcedIceAge();
} catch(err) {
  console.error('Ice Age check failed:', err);
}
updateNavLocks();
...
function getCookieTickGain(){
  if(typeof getPer40mEarned === 'function') return getPer40mEarned();
...
function runCookieTick(){
  try {
    if(!gamePaused){
      ...
      if(typeof tickGardenMutations === 'function') tickGardenMutations();
    }
  } finally {
...
function updateCookieTickTimer(){
  ...
  if(gamePaused){
    ...
    text = ms <= 0 ? "Now…" : formatCookieTickCountdown(ms);
  }
...
function showMenu(id){
  if(!isNavTabUnlocked(id)){
    showToast(`Unlocks at ${NAV_TORNADO_L2_LABEL}.`, "warn");
    return;
  }
  ...
  if(id === 'bank' && typeof updateBankUI === 'function') updateBankUI();
  if(id === 'upgrade-shop' && typeof updatePortalShopUI === 'function') updatePortalShopUI();
  if(id === 'ice-age' && typeof updateIceAgeUI === 'function') updateIceAgeUI();
  if(id === 'trade' && typeof updateTradeUI === 'function') updateTradeUI();
}

window.formatNum = formatNum;
window.showToast = showToast;
window.setSessionHeader = setSessionHeader;
window.setSaveIndicator = setSaveIndicator;
